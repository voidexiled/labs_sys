...
export default async function Page({
					...
					queryFn: async () => {
						...
						if (subjectData) {
							...
							await queryClient.prefetchQuery({
								...
								queryFn: async () => {
									const supabase = await createSupabaseServer();
									const { data: allCoursesData, error: allCoursesError } =
										await supabase
											.from("courses")
											.select("*")
											.eq("subject_id", subjectData.id);
									if (allCoursesData) {
										const allCoursesIds = allCoursesData.map(
											(course) => course.id,
										);
										...
										await queryClient.prefetchQuery({
											queryKey: [
												"subject_courses_units",
												params.courseId,
												params.unitId,
											],
											queryFn: async () => {
												const supabase = await createSupabaseServer();
												const { data: allUnitsData, error: allUnitsError } =
													await supabase
														.from("units")
														.select("*")
												...
												if (allUnitsData) {
													const allUnitsIds = allUnitsData.map(
														(unit) => unit.id,
													);
													...
													await queryClient.prefetchQuery({
														queryKey: [
															"subject_courses_units_assignments",
															params.courseId,
															params.unitId,
														],
														queryFn: async () => {
	...
	return (
		<HydrationBoundary state={dehydrate(queryClient)}>
			<GroupUnitContainer courseId={params.courseId} unitId={params.unitId} />
		</HydrationBoundary>
	);
}
