...
export default async function RegistrarGruposPage() {
    ...
    const { data: dataSubjects, error: SError } = await supabase.from("subjects").select("*");
    const { data: dataTeachers, error: TError } = await supabase.from("users").select("*").eq("role_id", 4);
    ...
    return (
        <MainWrapper>
            ...
            <MainWrapperContent>
                <GroupCreator subjects={dataSubjects as Tables<"subjects">[]} teachers={dataTeachers as Tables<"users">[]}></GroupCreator>
            </MainWrapperContent>
