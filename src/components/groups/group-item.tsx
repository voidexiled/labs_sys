...
export const GroupItem = ({
	...
	course: Tables<"courses"> | undefined;
	subject: Tables<"subjects"> | undefined;
	...
	return (
		<>
			{course && subject && (
				<Link
					key={course.id}
					href={`grupos/${course.id}`}
					className="group flex w-56 h-48 mx-auto"
				>
					<motion.div
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
					...
					>
						<div className="rounded-xl w-[76px] h-[76px] bg-secondary flex items-center justify-center p-1 border text-sm">
							{subject ? (
								<span>
									{subject.key} - {course.label}
								</span>
							) : (
								<Skeleton className="rounded-full object-cover w-full h-full" />
							)}
						</div>
						<span>{subject ? subject.label : "Cargando..."}</span>
					</motion.div>
				</Link>
			)}
		</>
