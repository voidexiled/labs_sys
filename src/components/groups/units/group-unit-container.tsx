...
export const GroupUnitContainer = ({
	courseId,
	unitId,
	className,
	children,
	...
	const {
		data: unitAssignmentsData,
		error: unitAssignmentsError,
		refetch: refetchUnitAssignments,
		isFetching: isFetchingUnitAssignments,
	} = useQuery({
	...
	const {
		...
		isFetching: isFetchingAllAssignments,
	} = useQuery({
		queryKey: ["subject_courses_units_assignments", courseId, unitId],
		queryFn: async () => {
			const supabase = createSupabaseBrowser();
			const allUnitsIds = allUnitsData?.map((unit) => unit.id);
			const { data: allAssignmentsData, error } = await supabase
				.from("assignments")
				.select("*")
				.in("unit_id", allUnitsIds!);
	...
	const isLoadingSomething =
		isFetchingUnitAssignments ||
		isFetchingCourse ||
		isFetchingSubject ||
		isFetchingAllCourses ||
		isFetchingAllUnits ||
		isFetchingAllAssignments;

	return (
		<motion.div
			className={cn(
				"flex flex-col items-start justify-start w-full h-full px-6 py-5  text-secondary-foreground",
				className,
			)}
		>
			{children ? (
