...
export default function LaboratoriesList({
	...
	useEffect(() => {
		if (filteredLaboratories) {
			...
			for (
				let i = 1;
				i <= Math.ceil(filteredLaboratories.length / itemsPerPage);
				i++
			) {
				temp.push(i);
			}
	...
	const filterLaboratories = () => {
		...
		return laboratories.filter((lab) => {
			...
			const teacherFilter = teacher
				? _course?.teacher_id === _teacher?.id
				: true;
			const subjectFilter = subject
				? _course?.subject_id === _subject?.id
				: true;
	...
	return (
		<>
			<ScrollAreaDashboard>
				{isFetchingLaboratories ||
				isFetchingSubjects ||
				isFetchingTypesUsers ||
				isFetchingUsers ||
				isFetchingCourses ? (
					...
					pagedLaboratories?.map((lab) => {
						const course = courses?.find(
							(course) => course.id === lab.course_id,
						) as Tables<"courses">;
						const subject =
							course &&
							(subjects?.find(
								(subject) => subject.id === course.subject_id,
							) as Tables<"subjects">);
						const teacher =
							course &&
							(users?.find(
