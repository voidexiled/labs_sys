...
const formSchema = z.object({
    ...
    classroom_code: z.string().min(10, { message: "Ingresa un código de clase válido." }).max(10, { message: "Ingresa un código de clase válido." }),
    subject_id: z.number().int().positive({ message: "Este campo es requerido." }),
    ...
    status: z.enum(["active", "inactive"], { message: "Este campo es requerido." }),
    type: z.enum(["presencial", "online", "mix"], { message: "Este campo es requerido." }),
    visibility: z.enum(["private", "public"], { message: "Este campo es requerido." }),
})
export default function GroupCreator({ subjects, teachers }: { subjects: Tables<"subjects">[], teachers: Tables<"users">[] }) {
    ...
    async function revertChanges(course_id: number) {
        ...
        const { data: DCData, error: DCError } = await supabase.from("courses").delete().eq("id", course_id);
        if (DCError) {
            console.error(DCError)
            toast({
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        const { data: CData, error: CError } = await supabase.from("courses").insert([
            {
                label: values.label,
        ...
        if (CData) {
            ...
            const { data: SData, error: SError } = await supabase.from("subjects").select("*").eq("id", values.subject_id).single();
            ...
            if (SError) {
                ...
                revertChanges(CData.id)
                return;
            }
            console.log(SData)

            if (SData && SData.syllabus_id) {
                const { data: SyllabusData, error: SyllabusError } = await supabase.from("syllabuses").select("*").eq("id", SData.syllabus_id).single();
                if (SyllabusError) {
                ...
                if (SyllabusData) {
                    ...
                    Array.from(new Array(SyllabusData.units)).map((unit: number, index: number) => {
                        units.push({
                            course_id: CData.id,
                            file_name: "",
                            unit: index + 1,
                    ...
                    const { data: UnitsData, error: UnitsError } = await supabase.from("units").insert(units).select("*");
                    if (UnitsError) {
                                    ...
                                    <PopoverTrigger asChild>
                                        <FormControl>
                                            <Button
                                                ...
                                                onClick={() => setIsOpenSubjectComboBox(!isOpenSubjectComboBox)}
                                            >
                                    ...
                                    <PopoverTrigger asChild>
                                        <FormControl>
                                            <Button
                                                ...
                                                onClick={() => setIsOpenTeacherComboBox(!isOpenTeacherComboBox)}
                                            >
                                                {/* Selecciona una materia */}
                                                {field.value
                                                    ? teachers.find(
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input type="number" placeholder="32" min={1} inputMode="numeric" {...field} className="w-[7.5rem]" onChange={(e) => {
                                        form.setValue("enrollment_limit", parseInt(e.target.value))
                                    }} />
                                </FormControl>
                                <FormDescription>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input type="date" min={minDate} className="w-fit" {...field} />
                                    {/* <DateInput
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="XXXXXXXXXX" {...field} className="w-[7.5rem]" />
                                </FormControl>
                                <FormDescription>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem >
                                ...
                                <FormDescription>
                                    Activo - es visible para el docente que impartirá y alumnos inscritos
                                    <br />
                                    Inactivo - solo es visible para los jefes (departamento, coordinación, etc.)
                                </FormDescription>
                                <FormMessage />
                            </FormItem>
                        )}
